...
df = pd.read_csv('./crawling_data_3/laftel_crawling_data_1_20221201.csv')
print(df.head())
df.info()
...
for i in range(len(tokened_X)):
    if len(tokened_X[i]) > 120:
        tokened_X[i] = tokened_X[i][:120] #50글자보다 크면 삭제한다
X_pad = pad_sequences(tokened_X, 120)

model = load_model('./models/laftel_category_calssfication_model_0.642.h5')
